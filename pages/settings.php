...
if ( isset( $_GET['message'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
    ...
    $sifp_text  = '';
    switch ( $sifp_msg ) {
        case 'settings_saved':    $sifp_text = __( 'Settings saved successfully!', 'si-fast-product-importer' ); break;
        case 'db_regenerated':    $sifp_text = __( 'Local database regenerated and synced!', 'si-fast-product-importer' ); break;
        case 'db_synced':         $sifp_text = __( 'Database file synced to table successfully!', 'si-fast-product-importer' ); break;
        case 'db_file_not_found': $sifp_text = __( 'Database file not found. Please regenerate.', 'si-fast-product-importer' ); $sifp_class = 'notice-error'; break;
    }
    if ( $sifp_text ) {
...
<div id="sifp-admin-content" class="sifp-main-container">
    ...
    <div class="sifp-settings-header">
        ...
        <h1><span class="dashicons dashicons-admin-settings"></span> <?php esc_html_e( 'Plugin Settings', 'si-fast-product-importer' ); ?></h1>
        <button form="sifp-form" name="update" value="update" class="sifp-button">
            <span class="dashicons dashicons-saved"></span>
            <?php esc_html_e( 'SAVE ALL SETTINGS', 'si-fast-product-importer' ); ?>
        </button>
    </div>

    <!-- Tabs -->
    <div class="sifp-settings-tabs">
        <a href="?page=si_fast_products_settings&tab=ai-providers"
            class="sifp-settings-tab <?php echo 'ai-providers' === $sifp_active_tab ? 'sifp-settings-tab--active' : ''; ?>"
            data-tab="ai-providers">
            <span class="dashicons dashicons-admin-appearance"></span>
            <?php esc_html_e( 'AI Providers', 'si-fast-product-importer' ); ?>
        </a>
        <a href="?page=si_fast_products_settings&tab=settings"
            class="sifp-settings-tab <?php echo 'settings' === $sifp_active_tab ? 'sifp-settings-tab--active' : ''; ?>"
            data-tab="settings">
            <span class="dashicons dashicons-admin-generic"></span>
            <?php esc_html_e( 'Settings', 'si-fast-product-importer' ); ?>
        </a>
        <a href="?page=si_fast_products_settings&tab=database"
            class="sifp-settings-tab <?php echo 'database' === $sifp_active_tab ? 'sifp-settings-tab--active' : ''; ?>"
            data-tab="database">
            <span class="dashicons dashicons-database"></span>
            <?php esc_html_e( 'Database', 'si-fast-product-importer' ); ?>
        </a>
    </div>

    <form id="sifp-form" method="post" class="sifp-form">
        <?php wp_nonce_field( 'si-fast-prod-sett', 'sett_nonce' ); ?>
        ...
        <div class="sifp-tab-panel" data-tab="ai-providers" <?php echo 'ai-providers' !== $sifp_active_tab ? 'style="display:none;"' : ''; ?>>
            ...
            <div class="sifp-provider-selector">
                <label for="sifp_active_ai_provider"><?php esc_html_e( 'Active AI Provider', 'si-fast-product-importer' ); ?></label>
                <?php
                ...
                <select name="setting[sifp_active_ai_provider]" id="sifp_active_ai_provider">
                    ...
                    ?>
                        <option value="<?php echo esc_attr( $sifp_pid ); ?>" <?php selected( $sifp_active_id, $sifp_pid ); ?>>
                            <?php echo esc_html( $sifp_pinst->get_name() ); ?> (<?php echo esc_html( $sifp_pcount ); ?> <?php esc_html_e( 'models', 'si-fast-product-importer' ); ?>)
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
                ...
                <div class="sifp-provider-card" id="sifp-provider-<?php echo esc_attr( $sifp_pid ); ?>" <?php echo $sifp_is_active ? '' : 'style="display:none;"'; ?>>
                    ...
                    <div class="sifp-provider-card__head">
                        ...
                        <button type="button" class="sifp-provider-card__refresh"
                                ...
                                title="<?php esc_attr_e( 'Refresh model list', 'si-fast-product-importer' ); ?>">
                    ...
                    <div class="sifp-provider-card__body">
                        ...
                        <div class="sifp-field">
                            <label class="sifp-field__label"><?php esc_html_e( 'Model', 'si-fast-product-importer' ); ?></label>
                            <div class="sifp-field__control">
                                <select name="setting[<?php echo esc_attr( $sifp_pinst->get_model_option_name() ); ?>]"
                                    ...
                                    <?php if ( ! empty( $sifp_premium_models ) ) : ?>
                                        <optgroup label="<?php esc_attr_e( 'Premium Models', 'si-fast-product-importer' ); ?>">
                                    ...
                                    <?php if ( ! empty( $sifp_free_models ) ) : ?>
                                        <optgroup label="<?php esc_attr_e( 'Free Models', 'si-fast-product-importer' ); ?>">
                                    ...
                                    <option value="custom" <?php selected( $sifp_saved_model, 'custom' ); ?>>
                                        <?php esc_html_e( 'Custom Model (enter below)', 'si-fast-product-importer' ); ?>
                                    </option>
                                </select>
                                <p class="sifp-field__hint">
                                    <?php esc_html_e( 'Custom model ID:', 'si-fast-product-importer' ); ?>
                                    <input type="text" name="setting[<?php echo esc_attr( $sifp_pinst->get_model_option_name() ); ?>_custom]"
                                        ...
                                        placeholder="<?php esc_attr_e( 'e.g. my-custom-model', 'si-fast-product-importer' ); ?>">
                                </p>
                            </div>
                        </div>
                        ...
                        <!-- API Key -->
                    <div class="sifp-field">
                        <label class="sifp-field__label"><?php esc_html_e( 'API Key', 'si-fast-product-importer' ); ?></label>
                        <div class="sifp-field__control sifp-api-key-wrapper">
                            ...
                            <button type="button" class="sifp-button sifp-button--small sifp-test-connection-btn"
                                ...
                                <?php esc_html_e( 'Test Connection', 'si-fast-product-importer' ); ?>
                            </button>
                        </div>
                    </div>

                    <div class="sifp-field">
                        <label class="sifp-field__label"><?php esc_html_e( 'Custom Model (Optional)', 'si-fast-product-importer' ); ?></label>
                        <div class="sifp-field__control">
                            <input type="text" name="setting[<?php echo esc_attr( $sifp_pinst->get_model_option_name() . '_custom' ); ?>]"
                                    value="<?php echo esc_attr( $sifp_custom_model ); ?>"
                                    placeholder="<?php esc_attr_e( 'e.g. gpt-4-turbo', 'si-fast-product-importer' ); ?>">
                        </div>
                    </div>
                        </div>
                        ...
                        <div class="sifp-field sifp-field--test">
                            <label class="sifp-field__label"><?php esc_html_e( 'Connection', 'si-fast-product-importer' ); ?></label>
                            <div class="sifp-field__control">
        ...
        <div class="sifp-tab-panel" data-tab="settings" <?php echo 'settings' !== $sifp_active_tab ? 'style="display:none;"' : ''; ?>>
            ...
            <div class="sifp-settings-section">
                <div class="sifp-settings-section__title"><?php esc_html_e( 'Product Defaults', 'si-fast-product-importer' ); ?></div>
                <div class="sifp-settings-section__body">

                    <div class="sifp-settings-row">
                        <div class="sifp-settings-row__label"><?php esc_html_e( 'SKU Prefix', 'si-fast-product-importer' ); ?></div>
                        <div class="sifp-settings-row__field">
                            ...
                            <p class="sifp-settings-row__hint"><?php esc_html_e( 'Prefix used for automatic SKU generation', 'si-fast-product-importer' ); ?></p>
                        </div>
                    </div>

                    <div class="sifp-settings-row">
                        <div class="sifp-settings-row__label"><?php esc_html_e( 'Default Stock', 'si-fast-product-importer' ); ?></div>
                        <div class="sifp-settings-row__field">
                            <input type="number" name="setting[sifp_default_stock]" value="<?php echo esc_attr( get_option( 'sifp_default_stock', '10' ) ); ?>" min="0">
                            <p class="sifp-settings-row__hint"><?php esc_html_e( 'Stock quantity assigned to imported products', 'si-fast-product-importer' ); ?></p>
                        </div>
                    </div>

                    <div class="sifp-settings-row">
                        <div class="sifp-settings-row__label"><?php esc_html_e( 'Product Status', 'si-fast-product-importer' ); ?></div>
                        <div class="sifp-settings-row__field">
                            <select name="setting[sifp_default_product_status]">
                                <option value="publish" <?php selected( get_option( 'sifp_default_product_status', 'publish' ), 'publish' ); ?>><?php esc_html_e( 'Published', 'si-fast-product-importer' ); ?></option>
                                <option value="draft" <?php selected( get_option( 'sifp_default_product_status', 'publish' ), 'draft' ); ?>><?php esc_html_e( 'Draft', 'si-fast-product-importer' ); ?></option>
                                <option value="pending" <?php selected( get_option( 'sifp_default_product_status', 'publish' ), 'pending' ); ?>><?php esc_html_e( 'Pending Review', 'si-fast-product-importer' ); ?></option>
                            </select>
                            <p class="sifp-settings-row__hint"><?php esc_html_e( 'Default status for imported/generated products', 'si-fast-product-importer' ); ?></p>
                        </div>
                    </div>

                    <div class="sifp-settings-row">
                        <div class="sifp-settings-row__label"><?php esc_html_e( 'Menu Position', 'si-fast-product-importer' ); ?></div>
                        <div class="sifp-settings-row__field">
                            <input type="number" name="setting[sifp_menu_order]" value="<?php echo esc_attr( get_option( 'sifp_menu_order', '15' ) ); ?>" min="1" max="100">
                            <p class="sifp-settings-row__hint"><?php esc_html_e( 'Position of the plugin in the WordPress admin menu', 'si-fast-product-importer' ); ?></p>
                        </div>
                    </div>
            ...
            <!-- AI Generation -->
            <div class="sifp-settings-section">
                <div class="sifp-settings-section__title"><?php esc_html_e( 'AI Generation', 'si-fast-product-importer' ); ?></div>
                <div class="sifp-settings-section__body">

                    <div class="sifp-settings-row">
                        <div class="sifp-settings-row__label"><?php esc_html_e( 'Creativity', 'si-fast-product-importer' ); ?></div>
                        <div class="sifp-settings-row__field">
                            ...
                            <p class="sifp-settings-row__hint"><?php esc_html_e( 'Lower = more precise, higher = more creative', 'si-fast-product-importer' ); ?></p>
                        </div>
                    </div>

                    <div class="sifp-settings-row">
                        <div class="sifp-settings-row__label"><?php esc_html_e( 'Tone Instruction', 'si-fast-product-importer' ); ?></div>
                        <div class="sifp-settings-row__field">
                            <input type="text" name="setting[sifp_ai_tone]" value="<?php echo esc_attr( get_option( 'sifp_ai_tone', '' ) ); ?>" placeholder="<?php esc_attr_e( 'e.g. Professional, Persuasive, Technical', 'si-fast-product-importer' ); ?>">
                            <p class="sifp-settings-row__hint"><?php esc_html_e( 'Global instruction for the AI voice tone', 'si-fast-product-importer' ); ?></p>
                        </div>
                    </div>
            ...
            <!-- Remote Databases -->
            <div class="sifp-settings-section">
                <div class="sifp-settings-section__title"><?php esc_html_e( 'Remote Databases', 'si-fast-product-importer' ); ?></div>
                <div class="sifp-settings-section__body">

                    <div class="sifp-settings-row">
                        <div class="sifp-settings-row__label"><?php esc_html_e( 'Database URLs', 'si-fast-product-importer' ); ?></div>
                        <div class="sifp-settings-row__field">
                            ...
                            <p class="sifp-settings-row__hint"><?php esc_html_e( 'One URL per line. Each endpoint must return a JSON with a "result" array of products.', 'si-fast-product-importer' ); ?></p>
                        </div>
                    </div>

                    <details style="padding:12px 22px;cursor:pointer;">
                        <summary style="color:var(--sifp-primary);font-weight:600;"><?php esc_html_e( 'View JSON example', 'si-fast-product-importer' ); ?></summary>
    ...
    "result": [
        ...
        },
        {
            ...
            <div class="sifp-settings-section">
                <div class="sifp-settings-section__title">
                    <span><?php esc_html_e( 'Error Logs', 'si-fast-product-importer' ); ?></span>
                    <button type="button" class="sifp-button sifp-button--small sifp-button--error clear-logs-btn">
                        <span class="dashicons dashicons-trash"></span>
                        <?php esc_html_e( 'Clear Logs', 'si-fast-product-importer' ); ?>
                    </button>
                </div>
                <div class="sifp-settings-section__body sifp-settings-section__body--nopad">
                    <div class="sifp-log-table-container">
                        <table class="wp-list-table widefat fixed striped">
                            <thead>
                                <tr>
                                    <th class="sifp-log-th-time"><?php esc_html_e( 'Timestamp', 'si-fast-product-importer' ); ?></th>
                                    <th class="sifp-log-th-context"><?php esc_html_e( 'Context', 'si-fast-product-importer' ); ?></th>
                                    <th class="sifp-log-th-message"><?php esc_html_e( 'Message', 'si-fast-product-importer' ); ?></th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                ...
                                if ( empty( $sifp_logs ) ) {
                                    echo '<tr><td colspan="3" class="sifp-no-logs">' . esc_html__( 'No logs found.', 'si-fast-product-importer' ) . '</td></tr>';
                                } else {
        ...
        <div class="sifp-tab-panel" data-tab="database" <?php echo 'database' !== $sifp_active_tab ? 'style="display:none;"' : ''; ?>>
            ...
            <div class="sifp-settings-section">
                <div class="sifp-settings-section__title"><?php esc_html_e( 'Local Database', 'si-fast-product-importer' ); ?></div>
                <div class="sifp-settings-section__body">

                    <p class="sifp-db-intro"><?php esc_html_e( 'The local database contains demo products that you can search and import from the main page.', 'si-fast-product-importer' ); ?></p>

                    <?php
                    $sifp_upload_dir = wp_upload_dir();
                    $sifp_json_path  = $sifp_upload_dir['basedir'] . '/si-fast-product-importer/local_products.json';
                    ...
                    <div class="sifp-db-cards">
                        <div class="sifp-db-card <?php echo $sifp_has_file ? 'sifp-db-card--ok' : 'sifp-db-card--empty'; ?>">
                            ...
                            <div class="sifp-db-card__info">
                                <strong><?php esc_html_e( 'JSON File', 'si-fast-product-importer' ); ?></strong>
                                <?php if ( $sifp_has_file ) : ?>
                                    <span class="sifp-db-card__status"><?php echo esc_html( size_format( filesize( $sifp_json_path ) ) ); ?></span>
                                <?php else : ?>
                                    <span class="sifp-db-card__status sifp-db-card__status--missing"><?php esc_html_e( 'Not found', 'si-fast-product-importer' ); ?></span>
                                <?php endif; ?>
                            </div>
                        </div>
                        ...
                        <div class="sifp-db-card <?php echo $sifp_has_db ? 'sifp-db-card--ok' : 'sifp-db-card--empty'; ?>">
                            ...
                            <div class="sifp-db-card__info">
                                <strong><?php esc_html_e( 'DB Table', 'si-fast-product-importer' ); ?></strong>
                                <?php if ( $sifp_has_db ) : ?>
                                    <span class="sifp-db-card__status"><?php echo sprintf(
                                        /* translators: %d: number of products in the database */
                                        esc_html__( '%d products', 'si-fast-product-importer' ),
                                        intval( $sifp_count )
                                    ); ?></span>
                                <?php else : ?>
                                    <span class="sifp-db-card__status sifp-db-card__status--missing"><?php esc_html_e( 'Empty', 'si-fast-product-importer' ); ?></span>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="sifp-db-actions">
                        <a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin.php?page=si_fast_products_settings&tab=database&sifp_regenerate_db=1' ), 'sifp_regenerate_db' ) ); ?>" class="sifp-button sifp-button--secondary">
                            <span class="dashicons dashicons-update"></span>
                            <?php esc_html_e( 'Regenerate Product DB', 'si-fast-product-importer' ); ?>
                        </a>
                        <a href="<?php echo esc_url( wp_nonce_url( admin_url( 'admin.php?page=si_fast_products_settings&tab=database&sifp_sync_db=1' ), 'sifp_sync_db' ) ); ?>" class="sifp-button sifp-button--secondary">
                            <span class="dashicons dashicons-database-import"></span>
                            <?php esc_html_e( 'Sync File to DB Table', 'si-fast-product-importer' ); ?>
                        </a>
                    </div>
